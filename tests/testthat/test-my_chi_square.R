test_that("my_chi_square returns correct class and structure", {
  res <- my_chi_square(test_data, plot = FALSE)
...
test_that("my_chi_square observed is a table", {
  res <- my_chi_square(test_data, plot = FALSE)
  expect_true(is.table(res$observed))
})

test_that("my_chi_square observed row sums match gender counts", {
  res   <- my_chi_square(test_data, plot = FALSE)
  clean <- test_data
...
test_that("my_chi_square NA in phys is treated as None", {
  d_with_na <- test_data
  d_with_na$phys[1:5] <- NA
  res <- my_chi_square(d_with_na, plot = FALSE)
  # "None" column must exist in observed table
  expect_true("None" %in% colnames(res$observed))
})

test_that("my_chi_square min_expected is positive", {
  res <- my_chi_square(test_data, plot = FALSE)
  expect_gt(res$min_expected, 0)
})

test_that("my_chi_square decision matches p_value vs alpha", {
  res <- my_chi_square(test_data, alpha = 0.05, plot = FALSE)
  expected_dec <- if (res$p_value < 0.05) "Reject H0" else "Fail to reject H0"
  expect_equal(res$decision, expected_dec)
})

test_that("my_chi_square errors on missing columns", {
  bad <- test_data[, c("gender", "height")]
  expect_error(my_chi_square(bad, plot = FALSE))
})

test_that("my_chi_square plots list contains a ggplot object", {
  res <- my_chi_square(test_data, plot = FALSE)
